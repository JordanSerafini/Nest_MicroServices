...
import { PurchaseDocumentLine } from "../../@types/purchases/PurchaseDocumentLine.entity";
import { getPurchaseById } from "../../utils/functions/purchase.function";
...
function AchatDetail() {
  ...
  const [docLines, setDocLines] = useState<PurchaseDocumentLine[]>([]);
  ...
  const fetchDocAndLines = async (DocumentId: string) => {
    try {
      const response = await getPurchaseById(DocumentId);
      
        const responseDoc = response.purchaseDocument;
        let responselines = response.purchaseDocumentLine;

      // Filtrer les lignes ayant quantité 0 et sans description
    //   responselines = filterLines(responselines);

      setPurchase(responseDoc);
  ...
  const filterLines = (lines: PurchaseDocumentLine[]) => {
    return lines.filter(
      (line) =>
        line.Quantity !== 0 &&
      ...
      {loading ? (
        ...
        <View>
          <View className="pb-4 p-2">
            <Text>Notes: {purchase.NotesClear}</Text>
            <Text>Document Number: {DocumentNumber}</Text>
            <Text>Fournisseur: {purchase.SupplierName}</Text>
            <Text>Date: {purchase.DocumentDate}</Text>
          </View>
